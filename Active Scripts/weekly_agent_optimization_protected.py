...
- 2+ weeks inactive: agent → light agent (updated from 1 week)
...
class WeeklyAgentOptimizer:
    def __init__(self, dry_run: bool = True):
        ...
        if status == 200:
            ...
            self.week_cutoff = datetime.now(timezone.utc) - timedelta(days=14)  # Changed to 2 weeks
            ...
            logging.info(f"📅 Two-week cutoff: {self.week_cutoff.strftime('%Y-%m-%d %H:%M:%S UTC')}")  # Updated label
        ...
        # Set up time calculations - Changed from 7 days to 14 days (2 weeks)
        self.week_cutoff = datetime.now(timezone.utc) - timedelta(days=14)
        self.month_cutoff = datetime.now(timezone.utc) - timedelta(days=30)
    ...
    def analyze_user_activity(self, team_members: List[Dict]) -> Dict:
        ...
        for user in team_members:
            try:
                ...
                # Categorize based on activity - Updated to use 14 days instead of 7
                if days_since is not None:
                    if days_since >= 30:  # Month+ inactive
                        if user_data['role'] in ['agent', 'end-user']:
                            ...
                            results['errors'].append(user_data)
                    elif days_since >= 14:  # Two weeks+ inactive (updated from 7 days)
        ...
        # Log summary - Updated labels to reflect 2 weeks
        logging.info("📈 Activity Analysis Summary:")
        logging.info(f"   🟢 Active (< 14 days): {len(results['active'])}")
        logging.info(f"   🟡 Two weeks inactive (14-29 days): {len(results['week_inactive'])}")
    ...
    def run_optimization(self):
        ...
        try:
            ...
            logging.info("🔄 Converting two-week-inactive agents to light agents...")  # Updated label
